...
public abstract class ServerConfigurationManager {
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn) {
        ...
        ChatComponentTranslation chatcomponenttranslation = !playerIn.getCommandSenderName().equalsIgnoreCase(s) ? new ChatComponentTranslation("multiplayer.player.joined.renamed", playerIn.getDisplayName(), s) : new ChatComponentTranslation("multiplayer.player.joined", playerIn.getDisplayName());
    ...
    public EntityPlayerMP getPlayerByUsername(String username) {
        for (EntityPlayerMP entityplayermp : this.playerEntityList) {
            if (!entityplayermp.getCommandSenderName().equalsIgnoreCase(username)) continue;
            return entityplayermp;
        }
        return null;
    }
    ...
    public StatisticsFile getPlayerStatsFile(EntityPlayer playerIn) {
        ...
        if (statisticsfile == null) {
            ...
            if (!file2.exists() && (file3 = new File(file1, playerIn.getCommandSenderName() + ".json")).exists() && file3.isFile()) {
