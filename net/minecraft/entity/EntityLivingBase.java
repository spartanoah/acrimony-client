...
extends Entity {
    ...
    private int getArmSwingAnimationEnd() {
        Tickbase tickbase = Acrimony.instance.getModuleManager().getModule(Tickbase.class);
        if (tickbase.freezing) {
            return 0;
        }
        Animations animationsModule = Acrimony.instance.getModuleManager().getModule(Animations.class);
        if (animationsModule.isEnabled()) {
            return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (int)((double)(this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6) * Animations.sSpeed.getValue() * (double)Minecraft.getMinecraft().timer.timerSpeed);
        }
    ...
    protected void jump() {
        double jumpMotion = this.getJumpUpwardsMotion();
        if (this.isPotionActive(Potion.jump)) {
            jumpMotion += (double)((float)(this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1f);
        }
        JumpEvent event = new JumpEvent(jumpMotion, this.rotationYaw, this.isSprinting(), 0.2f);
        ...
        if (event.isBoosting()) {
            ...
            this.motionX -= (double)(MathHelper.sin(f) * event.getBoostAmount());
            this.motionZ += (double)(MathHelper.cos(f) * event.getBoostAmount());
        }
        this.isAirBorne = true;
    }
    ...
    public void onLivingUpdate() {
        ...
        double d = Math.abs(this.motionX);
        double d2 = ViaLoadingBase.getInstance().getTargetVersion().getVersion() > ProtocolVersion.v1_8.getVersion() ? 0.003 : 0.005;
        if (d < d2) {
            this.motionX = 0.0;
        }
        double d3 = Math.abs(this.motionY);
        double d4 = ViaLoadingBase.getInstance().getTargetVersion().getVersion() > ProtocolVersion.v1_8.getVersion() ? 0.003 : 0.005;
        if (d3 < d4) {
            this.motionY = 0.0;
        }
        double d5 = Math.abs(this.motionZ);
        double d6 = ViaLoadingBase.getInstance().getTargetVersion().getVersion() > ProtocolVersion.v1_8.getVersion() ? 0.003 : 0.005;
        if (d5 < d6) {
            this.motionZ = 0.0;
        }
