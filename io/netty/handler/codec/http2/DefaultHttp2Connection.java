...
implements Http2Connection {
    ...
    public Future<Void> close(Promise<Void> promise) {
        ...
        Iterator<IntObjectMap.Entry<Http2Stream>> itr = this.streamMap.entries().iterator();
        if (this.activeStreams.allowModifications()) {
            this.activeStreams.incrementPendingIterations();
            try {
                while (itr.hasNext()) {
                    DefaultStream stream = (DefaultStream)((IntObjectMap.PrimitiveEntry)((Object)itr.next())).value();
            ...
            while (itr.hasNext()) {
                Http2Stream stream = (Http2Stream)((IntObjectMap.PrimitiveEntry)((Object)itr.next())).value();
                if (stream.id() == 0) continue;
                stream.close();
            }
        }
