    ...
    implements PriorityQueueNode {
        ...
        void takeChild(Iterator<IntObjectMap.PrimitiveEntry<State>> childItr, State child, boolean exclusive, List<ParentChangedEvent> events) {
            ...
            if (exclusive && !this.children.isEmpty()) {
                Iterator<IntObjectMap.Entry<State>> itr = this.removeAllChildrenExcept(child).entries().iterator();
                while (itr.hasNext()) {
                    child.takeChild(itr, (State)((IntObjectMap.PrimitiveEntry)((Object)itr.next())).value(), false, events);
                }
            }
        }
        ...
        void removeChild(State child) {
            if (this.children.remove(child.streamId) != null) {
                ...
                if (!child.children.isEmpty()) {
                    Iterator<IntObjectMap.Entry<State>> itr = child.children.entries().iterator();
                    long totalWeight = child.getTotalWeight();
                    do {
                        State dependency = (State)((IntObjectMap.PrimitiveEntry)((Object)itr.next())).value();
