...
public class TypeUtils {
    ...
    private static boolean isAssignable(Type type, ParameterizedType toParameterizedType, Map<TypeVariable<?>, Type> typeVarAssigns) {
        ...
        for (TypeVariable<?> var2 : toTypeVarAssigns.keySet()) {
            Type toTypeArg = TypeUtils.unrollVariableAssignments(var2, toTypeVarAssigns);
            Type fromTypeArg = TypeUtils.unrollVariableAssignments(var2, fromTypeVarAssigns);
    ...
    private static Type unrollVariableAssignments(TypeVariable<?> var2, Map<TypeVariable<?>, Type> typeVarAssigns) {
        Type result;
        while ((result = typeVarAssigns.get(var2)) instanceof TypeVariable && !result.equals(var2)) {
            var2 = (TypeVariable)result;
        }
        return result;
    }
    ...
    private static Type[] extractTypeArgumentsFrom(Map<TypeVariable<?>, Type> mappings, TypeVariable<?>[] variables) {
        ...
        int index = 0;
        for (TypeVariable<?> var2 : variables) {
            Validate.isTrue(mappings.containsKey(var2), "missing argument mapping for %s", TypeUtils.toString(var2));
            result[index++] = mappings.get(var2);
        }
        return result;
    }
    ...
    public static String toLongString(TypeVariable<?> var2) {
        StringBuilder buf;
        block5: {
            Validate.notNull(var2, "var is null", new Object[0]);
            buf = new StringBuilder();
            Object d = var2.getGenericDeclaration();
        ...
        return buf.append(':').append(TypeUtils.typeVariableToString(var2)).toString();
    }
    ...
    private static String wildcardTypeToString(WildcardType w) {
        ...
        } else if (upperBounds.length != 1 || !Object.class.equals((Object)upperBounds[0])) {
