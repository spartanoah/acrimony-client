...
public class Digest {
    ...
    private void run(String[] digestAlgorithms) throws IOException {
        for (String messageDigestAlgorithm : digestAlgorithms) {
            if (!DigestUtils.isAvailable((String)messageDigestAlgorithm)) continue;
    ...
    private void run(String prefix, MessageDigest messageDigest) throws IOException {
        ...
        for (String source : this.inputs) {
            ...
            if (file.isFile()) {
                this.println(prefix, DigestUtils.digest((MessageDigest)messageDigest, (File)file), source);
                continue;
            }
            if (file.isDirectory()) {
            ...
            this.println(prefix, DigestUtils.digest((MessageDigest)messageDigest, (byte[])bytes));
        }
    }
    ...
    private void run(String prefix, MessageDigest messageDigest, File[] files) throws IOException {
        for (File file : files) {
            ...
            this.println(prefix, DigestUtils.digest((MessageDigest)messageDigest, (File)file), file.getName());
        }
    }
