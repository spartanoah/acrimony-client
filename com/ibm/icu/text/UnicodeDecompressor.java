...
implements SCSU {
    ...
    public static String decompress(byte[] buffer) {
        char[] buf = UnicodeDecompressor.decompress(buffer, 0, buffer.length);
    ...
    public int decompress(byte[] byteBuffer, int byteBufferStart, int byteBufferLimit, int[] bytesRead, char[] charBuffer, int charBufferStart, int charBufferLimit) {
        ...
        if (this.fBufferLength > 0) {
            ...
            int count = this.decompress(this.fBuffer, 0, this.fBuffer.length, null, charBuffer, charBufferStart, charBufferLimit);
            ucPos += count;
            bytePos += newBytes;
        }
        block20: while (bytePos < byteBufferLimit && ucPos < charBufferLimit) {
            block38: {
                block39: {
                    switch (this.fMode) {
                        case 0: {
                ...
                block22: while (bytePos < byteBufferLimit && ucPos < charBufferLimit) {
                    aByte = byteBuffer[bytePos++] & 0xFF;
                    switch (aByte) {
                        case 232: 
                        case 233: 
