...
implements PeriodBuilderFactory {
    ...
    class Settings {
        ...
        Settings setMaxLimit(float maxLimit) {
            int val2;
            int n = val2 = maxLimit <= 0.0f ? 0 : (int)(maxLimit * 1000.0f);
            if (maxLimit == (float)val2) {
                return this;
            }
            Settings result = this.inUse ? this.copy() : this;
            result.maxLimit = val2;
            return result;
        }

        Settings setMinLimit(float minLimit) {
            int val2;
            int n = val2 = minLimit <= 0.0f ? 0 : (int)(minLimit * 1000.0f);
            if (minLimit == (float)val2) {
                return this;
            }
            Settings result = this.inUse ? this.copy() : this;
            result.minLimit = val2;
            return result;
        }
