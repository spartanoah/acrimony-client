...
public final class InternalLocaleBuilder {
    ...
    public InternalLocaleBuilder setVariant(String variant) throws LocaleSyntaxException {
        if (variant == null || variant.length() == 0) {
            ...
            String var2 = variant.replaceAll("-", "_");
            int errIdx = this.checkVariants(var2, "_");
            if (errIdx != -1) {
    ...
    public InternalLocaleBuilder setExtension(char singleton, String value) throws LocaleSyntaxException {
        ...
        if (remove) {
            ...
            StringTokenIterator itr = new StringTokenIterator(val2, "-");
            while (!itr.isDone()) {
                String s = itr.current();
            ...
            if (UnicodeLocaleExtension.isSingletonChar(key.value())) {
                this.setUnicodeLocaleExtension(val2);
            } else {
                if (this._extensions == null) {
                    this._extensions = new HashMap(4);
                }
                this._extensions.put(key, val2);
            }
        }
        return this;
    }
    ...
    public InternalLocaleBuilder setLanguageTag(LanguageTag langtag) {
        ...
        if (bcpVariants.size() > 0) {
            StringBuilder var2 = new StringBuilder(bcpVariants.get(0));
            for (int i = 1; i < bcpVariants.size(); ++i) {
                var2.append("_").append(bcpVariants.get(i));
            }
            this._variant = var2.toString();
        }
