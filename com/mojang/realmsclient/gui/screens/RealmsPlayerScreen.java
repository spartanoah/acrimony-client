...
extends RealmsScreen {
    ...
    private void op(int index) {
        ...
        String selectedInvite = this.serverData.players.get(index).getUuid();
        try {
    ...
    public void render(int xm, int ym, float a) {
        ...
        t.vertex(0.0, this.height(), 0.0).tex(0.0, (float)(this.height() - bottom_border) / 32.0f + 0.0f).color(64, 64, 64, 255).endVertex();
        t.vertex(this.width(), this.height(), 0.0).tex((float)this.width() / 32.0f, (float)(this.height() - bottom_border) / 32.0f + 0.0f).color(64, 64, 64, 255).endVertex();
        t.vertex(this.width(), bottom_border, 0.0).tex((float)this.width() / 32.0f, 0.0).color(64, 64, 64, 255).endVertex();
    ...
    extends RealmsClickableScrolledSelectionList {
        ...
        protected void renderItem(int i, int x, int y, int h, Tezzelator t, int mouseX, int mouseY) {
            ...
            if (i < ((RealmsPlayerScreen)RealmsPlayerScreen.this).serverData.players.size()) {
                this.renderInvitedItem(i, x, y, h);
            }
        }

        private void renderInvitedItem(int i, int x, int y, int h) {
            PlayerInfo invited = ((RealmsPlayerScreen)RealmsPlayerScreen.this).serverData.players.get(i);
            RealmsPlayerScreen.this.drawString(invited.getName(), RealmsPlayerScreen.this.column1_x + 3 + 12, y + 1, invited.getAccepted() ? 0xFFFFFF : 0xA0A0A0);
