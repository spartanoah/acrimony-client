...
implements Comparable<UnsignedInteger> {
    ...
    @CheckReturnValue
    public UnsignedInteger plus(UnsignedInteger val2) {
        return UnsignedInteger.fromIntBits(this.value + Preconditions.checkNotNull(val2).value);
    }

    @CheckReturnValue
    public UnsignedInteger minus(UnsignedInteger val2) {
        return UnsignedInteger.fromIntBits(this.value - Preconditions.checkNotNull(val2).value);
    }

    @CheckReturnValue
    @GwtIncompatible(value="Does not truncate correctly")
    public UnsignedInteger times(UnsignedInteger val2) {
        return UnsignedInteger.fromIntBits(this.value * Preconditions.checkNotNull(val2).value);
    }

    @CheckReturnValue
    public UnsignedInteger dividedBy(UnsignedInteger val2) {
        return UnsignedInteger.fromIntBits(UnsignedInts.divide(this.value, Preconditions.checkNotNull(val2).value));
    }

    @CheckReturnValue
    public UnsignedInteger mod(UnsignedInteger val2) {
        return UnsignedInteger.fromIntBits(UnsignedInts.remainder(this.value, Preconditions.checkNotNull(val2).value));
    }
